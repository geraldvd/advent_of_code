...
def _(input_data, rules):
    def problem_b(data):
        ...
        assert min([len(s) for s in rule31 + rule42]) == max([len(s) for s in rule31 + rule42]), "Expected same length for all rules."
        ...
        for m in data["messages"]:
            ...
            if len(m) % rule_length == 0 and \
               m[:rule_length] in rule42 and \
               m[rule_length:2*rule_length] in rule42 and \
               m[-rule_length:] in rule31:
