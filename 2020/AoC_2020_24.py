...
def _(input_data):
    def parse_direction_string(s):
        return s.replace('se', 'se,').replace('sw', 'sw,').replace('nw', 'nw,').replace('ne', 'ne,') \
                ...
                .replace('ew', 'e,w').replace('we', 'w,e').replace('ee', 'e,e').replace('ww', 'w,w') \
                .replace('ee', 'e,e').replace('ww', 'w,w').strip(',').split(',')

    def norm_tile_tuple(tile):
        min_ns = min(tile[0], tile[2])
        min_ew = min(tile[1], tile[3])
        return (tile[0]-min_ns, tile[1]-min_ew, tile[2]-min_ns, tile[3]-min_ew)
    ...
    def problem_b(data):
        ...
        for day in range(10):
            ...
            for t in tiles.keys():
                ...
                if tiles[t] and (neighbours_black_count == 0 or neighbours_black_count > 2):
