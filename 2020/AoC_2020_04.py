...
def _(input_data):
    def problem_a(data):
        ...
        valid_passports = 0
        for d in data:
            valid_passports += sum([r in d.keys() for r in required_keys]) == len(
                required_keys
            )
...
def _(input_data, re):
    def problem_b(data):
        ...
        for d in data:
            ...
            hcl = (
                "hcl" in d.keys()
                and re.search(r"^#[0-9a-f]{6}$", d["hcl"]) is not None
            )
            ecl = "ecl" in d.keys() and d["ecl"] in [
                "amb",
                "blu",
                "brn",
                "gry",
