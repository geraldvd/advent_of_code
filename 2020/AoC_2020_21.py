...
def _(os, sample):
    ...
    def post_process(data):
        ...
        for d in data:
            ...
            food_dict = {"ingredients": ingredients.split(" "), "allergens": allergens.split(", ")}
    ...
    def calculate_allergen_ingredients(data):
        ...
        for ingredients, allergens in data:
            for a in allergens:
                if a not in allergen_options.keys():
                    ...
                    allergen_options[a] = [i for i in allergen_options[a] if i in ingredients]
        ...
        while len(allergens_confirmed.keys()) != len(allergen_options.keys()):
            for allergen, ingredients in allergen_options.items():
                i_new = [i for i in ingredients if i not in allergens_confirmed.values()]
                if len(i_new) == 1:
...
def _(calculate_allergen_ingredients, input_data):
    def problem_b(data):
        ...
        return ",".join([i[1] for i in sorted(allergen_ingredients.items(), key=lambda l: l[0])])
