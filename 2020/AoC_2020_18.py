...
def _(input_data):
    ...
    def math(s, eval_func):
        ...
        while "(" in s:
            ...
            s = (
                s[:idx_last_open_bracket]
                + str(eval_func(s[idx_last_open_bracket + 1 : i]))
                + s[i + 1 :]
            )
        return int(s)
    ...
    def eval_no_brackets_b(s):
        ...
        for i, p in enumerate(plus_ops):
            ...
            if len(parts_after_addition) and parts_after_addition[-1].isdigit():
                parts_after_addition[-1] = str(
                    int(parts_after_addition[-1]) + int(parts[p + 1])
                )
            # If the last element in parts_after_addition is not a number, it means it's a *
            else:
                parts_after_addition.append(
                    str(int(parts[p - 1]) + int(parts[p + 1]))
                )
