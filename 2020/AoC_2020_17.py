...
def _(np, os, sample):
    ...
    def post_process(data):
        ...
        data = np.array([[int(i) for i in list(d.strip().replace('#', '1').replace('.', '0'))] for d in data])
        print(data)
        return data
                ...
                for y in range(y_min, y_max+1):
                    for z in range(z_min, z_max+1):
                        ...
                        if coord in active_cubes:
                            if sum([1 for n in neighbours if n in active_cubes]) in [2,3]:
                                new_active_cubes.add(coord)
                
                        # If a cube is inactive but exactly 3 of its neighbors are active, the cube becomes active. 
                        # Otherwise, the cube remains inactive.
                        else:
                            if sum([1 for n in neighbours if n in active_cubes]) == 3:
                                new_active_cubes.add(coord)
...
def _(coord_minmax, input_data, np):
    ...
    def problem_b(data):
        active_cubes = set([(int(c[0]), int(c[1]), 0, 0) for c in np.argwhere(data)])
        for i in range(6):
            x_min, x_max = coord_minmax(active_cubes, 0)
            y_min, y_max = coord_minmax(active_cubes, 1)
            z_min, z_max = coord_minmax(active_cubes, 2)
