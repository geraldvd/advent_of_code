...
def _(os, re, sample):
    ...
    def post_process(data):
        ...
        for r in rules.split('\n'):
            ...
            data["rules"][res[0]] = ((int(res[1]), int(res[2])), (int(res[3]), int(res[4])))
        data["your_ticket"] = tuple([int(y) for y in your_ticket.split("\n")[1].split(",")])
...
def _(input_data, math, sum_invalid_numbers):
    ...
    def problem_b(data):
        ...
        possible_rule_seqs = {k:list(range(len(data["your_ticket"]))) for k in data["rules"].keys()}

        # Extract valid tickets
        valid_tickets = [t for t in data["nearby_tickets"] if not sum_invalid_numbers(t, data["rules"])]
        ...
        for k in sorted(possible_rule_seqs.keys(), key=lambda x: len(possible_rule_seqs[x])):
