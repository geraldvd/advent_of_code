...
def _(input_data, np):
    ...
    def calculate_distances(data):
        ...
        distance_list = sorted([(i, v) for i, v in np.ndenumerate(distances) if v > 0], key=lambda x: x[1])
                ...
                for idx_c, c in enumerate(circuits):
                    if i[0] in c and i[1] in c:
                        ...
                        if set_to_merge is None:
                            ...
                            circuits[set_to_merge] = circuits[set_to_merge].union(c)
                            circuits[idx_c] = {}
                            break
                    elif i[1] in c:
...
def _(distance_list, input_data):
    def problem_b(data):
        ...
        for d in distance_list:
            ...
            if len([len(c) for c in circuits if len(c)]) == 1 and sum([len(c) for c in circuits if len(c)]) == len(data):
