...
def _(input_data):
    def area_list(data):
        ...
        for i in range(len(data)):
            for j in range(i+1, len(data)):
                areas.append(((i, j), (abs(data[i][0]-data[j][0]) + 1) * (abs(data[i][1]-data[j][1]) + 1)))
    ...
    def problem_b(data):
        ...
        for a in areas:
            ...
            for d in data:
                if d[0] in range(min(c0[0], c1[0])+1, max(c0[0], c1[0])) and d[1] in range(min(c0[1], c1[1])+1, max(c0[1], c1[1])):
            ...
            for i, d in enumerate(data):
                ...
                if r0[0] == r1[0]:
                    ...
                    if not(r0[0] <= min(c0[0], c1[0]) or r0[0] >= max(c0[0], c1[0]) or \
                        (r0[1] <= min(c0[1], c1[1]) and r1[1] <= min(c0[1], c1[1])) or \
                        (r0[1] >= max(c0[1], c1[1]) and r1[1] >= max(c0[1], c1[1]))):
                        valid = False
                        break
                # Horizontal edge piece
                elif r0[1] == r1[1]:
                    # Check: entirely below of rectangle OR entirely above OR entirely left OR entirely right
                    if not(r0[1] <= min(c0[1], c1[1]) or r0[1] >= max(c0[1], c1[1]) or \
                        (r0[0] <= min(c0[0], c1[0]) and r1[0] <= min(c0[0], c1[0])) or \
                        (r0[0] >= max(c0[0], c1[0]) and r1[0] >= max(c0[0], c1[0]))):
                        valid = False
                        break
                else:
